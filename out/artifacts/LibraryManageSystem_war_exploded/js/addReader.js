...
$(function () {
	...
	var postdata = "password="+ $.trim($("#addPassword").val())+"&name="+$.trim($("#addName").val())+"&phone="+ $.trim($("#addPhone").val())+"&readerType="+ $.trim($("#addreaderType").val())
	+"&email="+ $.trim($("#addEmail").val())+"&paperNO="+$.trim($("#addPaperNO").val());
	ajax(
    		  {
			  	method:'POST',
...
function validAddReader() {
    ...
    var newPwd = $.trim($("#addPassword").val());
    if (newPwd == "") {
        $('#newPwd').parent().addClass("has-error");
        $('#newPwd').next().text("请输入密码");
        $("#newPwd").next().show();
        flag = false;
